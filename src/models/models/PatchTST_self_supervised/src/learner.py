...
class Learner(GetAttr):
    ...
    def __init__(self, dls, model, 
        ...
        #print("type(dls.train):", type(dls.train))
        #print("type(dls.valid):", type(dls.valid))
        #print("type(dls.test):", type(dls.test))
    ...
    def fit(self, n_epochs, lr=None, cbs=None, do_valid=True):
        ...
        import time
        #print("Inside fit 2, this is the n_epoch", n_epochs)
        t_before_fit = time.time()
        self('before_fit')
        print(f"before_fit callbacks done ({time.time()-t_before_fit:.1f}s), starting epochs...", flush=True)
        try:
            for self.epoch in range(n_epochs):
                print(self.epoch, "Long time")        
                self('before_epoch')                     
    ...
    def _adapt_batch(self, batch):
        if isinstance(batch, dict):
            return batch["inputs_embeds"], batch["label"]
        if isinstance(batch, (tuple, list)):
            return batch
        raise TypeError(f"Unsupported batch type: {type(batch)}")

    def all_batches(self, type_):
        #print("Inside all batches")
        total_batches = len(self.dl)
        print("total batches:", total_batches if total_batches is not None else "unknown")

        for num, raw_batch in enumerate(self.dl):
            if num % 500 == 0:
                pass  #print("number of batch =", num)

            self.iter = num
            self.batch = self._adapt_batch(raw_batch)
    ...
    def train_step(self, batch):
        ...
        #self.xb, self.yb = self.dls._unpack_batch(batch)
        # forward
        #print("Forward pass")
    ...
    def lr_finder(self, start_lr=1e-7, end_lr=10, num_iter=100, step_mode='exp', show_plot=True, suggestion='valley'):                
        ...
        #fresh_train = self.dls.train_dataloader() if hasattr(self.dls, "train_dataloader") else self.dls.train
        #train_batches = self._count_batches(fresh_train)
        #train_batches = len(self.dls.train)
        #print(train_batches, "This is the training batch")
        #n_epochs = num_iter // train_batches + 1
        #n_epochs = num_iter 
        #train_batches = self._count_batches(self.dls.train)
        #print(train_batches, "This is the training batch")
        #n_epochs = num_iter // train_batches + 1
        ...
        for cb in self.cbs:
            ...
            if attr is not None:
                ...
                if _timed:
                    ...
                    if _elapsed > 0.1:
                        print(f"[rank {_rank}] before_fit: {type(cb).__name__} took {_elapsed:.1f}s", flush=True)
