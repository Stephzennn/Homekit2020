...
class TrackTrainingCB(Callback):
    ...
    def accumulate(self ):
        ...
        # Use self.yb (learner.yb) rather than the original batch yb: callbacks such as
        # PatchMaskCB overwrite learner.yb with the patched target (xb_patch) during
        # before_forward, so self.yb always has the same shape as self.pred.
        if self.batch_recorder['with_metrics']:
            self.preds.append(self.pred.detach().cpu())
            self.targs.append(self.yb.detach().cpu())
    ...
    def compute_scores(self):
        ...
        #print("Here is target and preds")
        #print("cb.preds type:", type(self.preds))
        #print("cb.targets type:", type(self.targs))

        #try:
        #    print("cb.preds shape:", self.preds.shape)
        #except Exception:
        #    print("cb.preds has no .shape")

        #try:
        #    print("cb.targets shape:", self.targs.shape)
        #except Exception:
        #    print("cb.targets has no .shape")
